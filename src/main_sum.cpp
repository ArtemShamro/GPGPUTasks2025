            ...
            if (algorithm == "CPU") {
                ...
                if (context.type() == gpu::Context::TypeCUDA) {
                    if (algorithm == "01 atomicAdd from each workItem") {
                        ...
                        sum_accum_gpu.fill(0);
                        cuda::sum_04_local_reduction(gpu::WorkSize(GROUP_SIZE, n), input_gpu, reduction_buffer2_gpu, sum_accum_gpu, n);
                        sum_accum_gpu.readN(&gpu_sum, 1);
                    } else if (algorithm == "05 reduction one kernel") {
                        reduction_buffer2_gpu.fill(0);
                        cuda::sum_05_reduction_one_kernel(gpu::WorkSize(GROUP_SIZE, n), input_gpu, reduction_buffer2_gpu, n);
